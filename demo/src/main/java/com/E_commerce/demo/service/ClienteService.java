...
public class ClienteService {
    ...
    public ClienteResponse me() {
        Long id = SecurityUtils.getClienteIdLogado();
    ...
    public Page<ClienteResponse> listar(Pageable pageable) {
        if (!SecurityUtils.isAdmin()) {
            throw new AcessoNegadoException("Apenas administradores podem listar clientes.");
        }
    ...
    public void excluir(Long id) {
        if (!SecurityUtils.isAdmin()) {
            throw new AcessoNegadoException("Apenas administradores podem excluir clientes.");
        }
        Cliente cliente = repository.findById(id)
                .orElseThrow(() -> new ClienteNaoEncontradoException(id));
    ...
    private void garantirAcessoAoCliente(Long id) {
        if (SecurityUtils.isAdmin()) {
            return;
        }
        if (!SecurityUtils.getClienteIdLogado().equals(id)) {
